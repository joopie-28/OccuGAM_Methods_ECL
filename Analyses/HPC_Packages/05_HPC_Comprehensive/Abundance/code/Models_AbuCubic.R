# Bayesian N-mixture model with cubic function of covariates (third degree polynomial)
...
model {
  ...
  for(i in 1:nsite) {
    ...
    log(lambda[i]) <-  b0 + b1 * DisCov[i] + b2 * DisCov2[i] + b3 * DisCov3[i] + bLandscape[Landscape[i]] + bYear[Year[i]]
