...
# Main Analysis Part 3b: Fit Abundance models with polynomials on HPC
...
# Used to run the Bayesian abundance GAMs on the HPC
...
fitabu.HPC <- function(sp.list, covariate,params.to.monitor){
  ...
  # Change to GAM
  if(slurm.model.type == 'GAM'){
    # In order to create a GAM, we leverage this nifty jagam function,
...
##----- Fit the Bayesian abundnace Model in JAGS
...
threshold_mod_summary <- readRDS('data/abundance_mod_summary.rds') 
