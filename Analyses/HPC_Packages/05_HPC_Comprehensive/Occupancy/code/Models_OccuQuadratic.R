# Bayesian Occupancy model with quadratic function of covariates (second degree polynomial)
...
model {
  ...
  for(i in 1:nsite) {
    ...
    logit(psi[i]) <- b0 + b1 * DisCov[i] + b2 * DisCov2[i] + bLandscape[Landscape[i]] + bYear[Year[i]]
