...
fitoccu.HPC <- function(sp.list, covariate,params.to.monitor){
...
DrawPosteriorJAGS.HPC <- function(JAGS_model, sp.list, covariate){
  ...
  ifelse(grepl("cubic", slurm.model, ignore.case = TRUE),
                    tmp_est <- plogis(intercept + as.matrix(newdat[c('DisCov', 'DisCov2', 'DisCov3')]) %*% t(mod_mcmc[,2:4])), 
                    ifelse(grepl("quadratic", slurm.model, ignore.case = TRUE),
                           tmp_est <- plogis(intercept + (as.matrix(data.frame(JAGS_model$Data_List[c('DisCov', 'DisCov2')])) %*% t(mod_mcmc[,2:3]))),
                           ifelse(grepl("linear", slurm.model, ignore.case = TRUE), 
                                  tmp_est <- plogis(intercept + (as.matrix(data.frame(JAGS_model$Data_List[c('DisCov')])) %*% t(mod_mcmc[,2]))),
                                  # GAM approach
...
slurm.model <- paste0('code/Models_Occu',slurm.model.type,'.R')  #'Analyses/Models/03c_Models_OccuCubic.R'
...
# Model fitting
mod <- fitoccu.HPC(sp.list, covariate, params.to.monitor)
