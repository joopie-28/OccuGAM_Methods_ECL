...
# Main Analysis Part 2a: Fit Occupancy Models on HPC (Unmarked with ^1 and ^2, and Bayesian Occupancy GAMs)
...
fitabuGAM.HPC <- function(sp.list, covariate){
  sp <- names(sp.list[1])
  ...
  print(paste0('Fitting the Bayesian Abundance GAM to ', sp, ' for ', n_distinct(Landscape), ' distinct landscapes'))
  ...
  # Load in custome function for EDF - does not work when you change order of tracked variables!!!
  ...
  ni <- 3000;  nt <- 5; nb <- 60; nc <- 4; na = NULL     # for now
...
DrawPosteriorJAGS.ABU.HPC <- function(JAGS_model, sp.list, covariate){
...
umf.list.abu <- readRDS("data/umflistocc.rds")

# Thin the UMF list based on chosen combination
sp.list <- umf.list.abu[sp]
...
# Model fitting
mod <- fitabuGAM.HPC(sp.list, covariate)

# Extract all the necessary parameters
mod.extract <- DrawPosteriorJAGS.ABU.HPC(mod, sp.list, covariate)

end = Sys.time()

print(paste("Finished running abundance GAMM model and derivative computation for: ", sp," - ",covariate, " at ", Sys.time(),
            ". This model took ", round(difftime(end, start, units = "hours"), 4), " hours to be completed.", sep = ""))

##---- to do, fix names to be abundance explicit
...
threshold_mod_summary <- readRDS('data/threshold_mod_summary.rds') # this might cause problems in. parallel environemnt
...
saveRDS(threshold_mod_summary, paste0('results/OCCUGAM_RDS/', covariate ,"/", modname,'_threshold_mod_summary.rds'))
...
mod <- fitabuGAM.HPC(sp.list, 'Avg_OP_percent_3km')
...
test<-DrawPosteriorJAGS.ABU.HPC(mod, sp.list, covariate = 'Avg_OP_percent_3km')

plot((test$Draws$Quantiles$`50%`)~test$Draws$Quantiles$cov, type='l', ylim =c(0,15))
polygon(c(test$Draws$Quantiles$cov, rev(test$Draws$Quantiles$cov)), 
        c(test$Draws$Quantiles$`94.5%`, rev(test$Draws$Quantiles$`5.5%`)), border=NA, col=alpha('lightgrey',0.2))
...
test.mod <- pcount(~num_cams_active_at_date~Avg_OP_percent_3km+I(Avg_OP_percent_3km^2)+I(Avg_OP_percent_3km^3)+I(Avg_OP_percent_3km^4) + (1|Landscape), data = umf.list.abu$`Macaca fascicularis`)

pred <- predict(test.mod, type='state', re.form=NA)
cov.pred <- umf.list.abu$`Macaca fascicularis`@siteCovs$Avg_OP_percent_3km

points((pred$Predicted)~cov.pred, type = 'p', ylim = c(0,2500), col=as.factor(Landscape),pch=19, add=T)
