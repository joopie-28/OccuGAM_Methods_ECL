### write a new function that takes in a name and a covariate and returns the comparison graphs
"/Users/sassen/Dropbox/Joop MSc QBIO project/Wildlife GAMs Methods Paper/GAMS Methods - Results/Abundance Models/01_HPC_Abundance_v300724/results"

PlotComparisonModels <- function(sp, cov, full.mod.summary,
...
#  cov <- "Avg_OP_percent_3km"
  ...
  if(type == 'occupancy'){
    ...
    for (name in names(formulae)) {
      ...
      #occu models
      models[[name]] <- occu(formulae[[name]], data = umf.list.occu[[sp]])
    }
    ...
    pred <- unmarked::predict(best.model, type='state', re.form=NA)
  ...
  if(type == 'abundance'){
    ...
    for (name in names(formulae)) {
      ...
      models[[name]] <- tryCatch({
        pcount(formulae[[name]], data = umf.list.abu[[sp]])
      }, error = function(e) {
...
PlotComparisonModels(sp = 'Prionailurus bengalensis',
                     cov = "Avg_human_footprint_3km",
                     full.mod.summary = full.mod.summary,
                     rds.path = "/Users/sassen/Dropbox/Joop MSc QBIO project/Wildlife GAMs Methods Paper/GAMS Methods - Results/Abundance Models/01_HPC_Abundance_v300724/results",
                     umf.list.abu = umf.list.abu,
                     umf.list.occu = umf.list.occu)
...
# Loop over to produce plots
sp.vec <- names(umf.list.abu)[-c(1,2)]
cov.vec <- c("Avg_human_footprint_3km","Avg_forest_cover_3km",
             'Avg_OP_percent_3km', "Avg_human_population_3km",
             'Avg_FLLI_3km')
...
for (sp in sp.vec){
  ...
  for(cov in cov.vec){
    ...
    pdf(paste0('Outputs/Abundance_Model_Plots/', sp, "-", cov, ".pdf"), 
        height = 10,
        width = 30)
    
    PlotComparisonModels(sp = sp,
                         cov = cov,
                         full.mod.summary = full.mod.summary,
                         rds.path = "/Users/sassen/Dropbox/Joop MSc QBIO project/Wildlife GAMs Methods Paper/GAMS Methods - Results/Abundance Models/01_HPC_Abundance_v300724/results",
                         umf.list.abu = umf.list.abu,
...
# run all models in ubms (1 - 3)

# save them

# compute AUC deviations for all 
# plot them
# when predicting, make sure to use new df



library(ubms)

test <- ubms::stan_occu(formulae[[3]], data = umf.list.occu[['Prionailurus bengalensis']])


ubms::plot_effects(test, submodel = 'state')

plot_residuals(test, submodel="state")

gof(test, draws=100, quiet=TRUE)

dic(test)
ubms::loo(test)

loo::waic(test@stanfit)


newdat <- data.frame(seq(min(umf.list.occu[[sp]]@siteCovs[cov]), max(umf.list.occu[[sp]]@siteCovs[cov]), by = 0.001))
colnames(newdat) <- cov
pred<-predict(test, submodel = 'state',newdata = newdat, re.form = NA)

pred$cov <- newdat$Avg_OP_percent_3km
pred <- pred[order(pred$cov),]
...
# Add the intervals
polygon(c(pred$cov, rev(pred$cov)),
        c(pred$`97.5%`, rev(pred$`2.5%`)),  border=NA, col=alpha('lightgrey',0.2))

# The final panel is t
